...
export default function Sidebar() {
  ...
  return (
    <aside
      ...
      className={`transition-all duration-300 bg-gradient-to-b from-[#f1f5f9] via-[#edf4fb] to-[#e6eef7] text-slate-800 flex flex-col h-full border-r-2 border-[#0d94e8]/70 shadow-[10px_0_35px_rgba(15,23,42,0.12)] ${
        sidebarAberta ? "w-72" : "w-20"
      }`}
    >
      <div
        className={`
          bg-gradient-to-br from-[#061f4a] via-[#082f6f] to-[#0f172a]
          text-white font-bold shadow-lg border-b border-white/10
      ...
      >
        <div className={`flex items-center ${sidebarAberta ? "justify-between" : "justify-center"}`}>
          <div className={`flex items-center ${sidebarAberta ? "gap-3" : "justify-center"}`}>
            <div className="h-12 w-12 rounded-2xl bg-blue-700/80 shadow-inner flex items-center justify-center text-white shrink-0">
             <img
      ...
      <nav
        className={`flex-1 py-5 overflow-y-auto text-slate-700 text-sm font-bold ${
      ...
      <div className={`${sidebarAberta ? "px-5" : "px-2"} py-4 border-t border-slate-300/70 bg-slate-100/70`}>
        <button
          onClick={logout}
          title="Sair"
          className={`group flex items-center gap-3 text-sm text-red-600 hover:text-red-700 font-extrabold rounded-2xl hover:bg-white hover:shadow-md transition-all ${
            sidebarAberta ? "justify-start px-3 py-3" : "justify-center w-full px-2 py-3"
          }`}
        >
          <span className="h-9 w-9 rounded-xl bg-white text-red-600 flex items-center justify-center shrink-0 border border-red-100 group-hover:bg-red-600 group-hover:text-white transition-all">
            <IconLogout />
          </span>
...
function SectionTitle({ label }) {
  return (
    <div className="px-2 pt-2 text-[12px] uppercase tracking-wide font-black text-slate-500">
      {label}
    </div>
  );
}
...
function SectionDivider() {
  return <div className="h-px bg-slate-300/70 mx-2 my-3" />;
}
      ...
      className={`
        ...
        ${
          active
            ...
            : "text-slate-700 hover:bg-white hover:text-blue-800 hover:shadow-md"
        }
      ...
      <span
        className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 transition-all ${
          active
            ...
            : "bg-white/75 text-slate-600 border border-slate-200 group-hover:bg-blue-700 group-hover:text-white group-hover:border-blue-700"
        }`}
      >
        {icon}
      </span>
  ...
  return (
    <div>
      <button
        ...
        title={label}
        className={`
          group flex items-center rounded-2xl text-slate-800 text-sm font-extrabold w-full
      ...
      >
        <span className={`flex items-center ${compact ? "justify-center" : "gap-3"}`}>
          <span className="h-10 w-10 rounded-xl bg-white/75 text-slate-600 border border-slate-200 flex items-center justify-center shrink-0 group-hover:bg-blue-700 group-hover:text-white group-hover:border-blue-700 transition-all">
            {icon}
          </span>
          {!compact && <span className="truncate">{label}</span>}
        </span>
        ...
        {!compact && (
          <span className={`text-sm text-slate-500 transition-transform duration-200 ${open ? "rotate-180" : ""}`}>
            ▾
          </span>
        )}
      </button>
...
function NestedButton({ icon, label, open, onClick }) {
  return (
    <button
      onClick={onClick}
      className="group flex items-center justify-between w-full px-3 py-2 rounded-xl text-slate-700 text-[13px] font-bold hover:bg-white hover:text-blue-800 transition-all duration-200"
    >
      <span className="flex items-center gap-2.5">
        <span className="h-7 w-7 rounded-lg bg-white text-slate-600 flex items-center justify-center shrink-0 border border-slate-200 group-hover:bg-blue-700 group-hover:text-white group-hover:border-blue-700 transition-all">
          {icon}
        </span>
        <span>{label}</span>
      </span>
...
function SubItem({ icon, label, onClick, compact = false, color = "blue" }) {
  const colorClass =
    color === "green"
      ? "text-emerald-600 border-emerald-100 group-hover:bg-emerald-600 group-hover:border-emerald-600"
      : color === "red"
      ? "text-red-600 border-red-100 group-hover:bg-red-600 group-hover:border-red-600"
      : "text-slate-600 border-slate-200 group-hover:bg-blue-700 group-hover:border-blue-700";
  ...
  return (
    <button
      ...
      title={label}
      className={`
        group flex items-center rounded-xl text-slate-700 text-[13px] font-bold
        hover:bg-white hover:text-blue-800
        transition-all duration-200
        ${compact ? "justify-center px-2 py-2 w-full" : "gap-2.5 px-3 py-2 w-full text-left"}
      `}
    >
      <span className={`h-7 w-7 rounded-lg bg-white flex items-center justify-center shrink-0 border group-hover:text-white transition-all ${colorClass}`}>
        {icon}
      </span>
      {!compact && <span className="truncate">{label}</span>}
    </button>
