...
import { useLocation } from "react-router-dom";
 ...
 export default function LancamentoContabilEditar() {

  const navigate = useNavigate();
  const location = useLocation(); // 🔥 PRIMEIRO
  const lancamentoId = location.state?.id; // 🔥 DEPOIS
...
// helperMsg = null ou { titulo: string, mensagem: string }
 ...
 async function carregar() {
  ...
  const r = await fetch(buildWebhookUrl("buscar_lote", {
   
    empresa_id:empresa_id,
      lote_id:lancamentoId,
  }));

  const json = await r.json();

  carregarLancamentoTela(json); // 🔥 aqui
}


useEffect(() => {
  if (!lancamentoId || contas.length === 0) return;
...
function montarHistoricoPorNomes(nomeDeb, nomeCred) {
 ...
 async function salvar() {
  ...
  try {
    const res = await fetch(buildWebhookUrl("edita_lancto_partida"), {
      method: "POST",
...
function carregarLancamentoTela(dados) {
  if (!dados || dados.length === 0) return;

 
  const j = dados[0]?.data; // 🔥 AQUI ESTÁ A CORREÇÃO
 
  // 🔹 IDs
  setDebitoId(j.debito_id);
  setCreditoId(j.credito_id);

  // 🔹 TEXTO (ESSENCIAL pro autocomplete funcionar)
  setDebitoTexto(`${j.debito_codigo} - ${j.debito_nome}`);
  setCreditoTexto(`${j.credito_codigo} - ${j.credito_nome}`);

  // 🔹 VALORES
  setValor(j.valor);
  setHistorico(j.historico);
 
  // 🔹 DATA (corrigindo timezone)
  if (j.data_mov) {
    const data = new Date(j.data_mov);
    const dataFormatada = data.toISOString().slice(0, 10);
    setDataLancto(dataFormatada);
  }

  // 🔹 OBJETOS (pra helper funcionar)
  const deb = contas.find(c => c.id == j.debito_id);
  const cred = contas.find(c => c.id == j.credito_id);

  if (deb) setDebitoConta(deb);
  if (cred) setCreditoConta(cred);
}
    ...
    <div className="max-w-2xl mx-auto p-2">
      ...
      <div className="bg-[#061f4aff] rounded-xl p-3 mb-4 text-white text-center"> 
            ...
            <h2 className="text-2xl font-bold mb-6 text-center text-white">
              ⚡ Lançamento Contábil (Edição) — Lote {lancamentoId || "???"}
            </h2>
